...
from of_msgs.msg import ToioCommand
...
class ToioCommander:
    def __init__(self):
        ...
        self.last_odom = None
        self.toio_command_sub = rospy.Subscriber("toio_command", ToioCommand, self.toioCommandCallback)
        self.last_toio_command = ToioCommand()
    ...
    def toioCommandCallback(self, msg):
        self.last_toio_command = msg
        self.command_enable = True
        if msg.type == ToioCommand.TYPE_GOTO_POSITION:
            self.goto_stage = 0
    ...
    def timerCallback(self, event):
        ...
        if(self.last_toio_command.type == ToioCommand.TYPE_GOTO_POSITION):
            finish, cmd_vel, marker = self.calcurateTwist(self.last_toio_command.goal, self.last_odom)
